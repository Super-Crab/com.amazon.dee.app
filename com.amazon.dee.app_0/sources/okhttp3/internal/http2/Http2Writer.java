...
public final class Http2Writer implements Closeable {
    ...
    public final synchronized void connectionPreface() throws IOException {
        if (!this.closed) {
            ...
            this.sink.mo12591write(Http2.CONNECTION_PREFACE);
            this.sink.flush();
            return;
        }
        throw new IOException("closed");
    ...
    public final void frameHeader(int i, int i2, int i3, int i4) throws IOException {
        ...
        if (!(i2 <= this.maxFrameSize)) {
            StringBuilder outline107 = GeneratedOutlineSupport1.outline107("FRAME_SIZE_ERROR length > ");
            outline107.append(this.maxFrameSize);
        ...
        if (z) {
            ...
            this.sink.mo12596writeByte(i3 & 255);
            this.sink.mo12596writeByte(i4 & 255);
            this.sink.mo12599writeInt(i & Integer.MAX_VALUE);
            return;
        }
        throw new IllegalArgumentException(GeneratedOutlineSupport1.outline49("reserved bit set: ", i).toString());
    }
    ...
    public final synchronized void goAway(int i, @NotNull ErrorCode errorCode, @NotNull byte[] debugData) throws IOException {
        ...
        if (!this.closed) {
            ...
            if (errorCode.getHttpCode() != -1) {
                ...
                this.sink.mo12599writeInt(i);
                this.sink.mo12599writeInt(errorCode.getHttpCode());
                if (debugData.length == 0) {
                    z = true;
                }
                if (!z) {
                    this.sink.mo12594write(debugData);
                }
                this.sink.flush();
            } else {
    ...
    public final synchronized void pushPromise(int i, int i2, @NotNull List<Header> requestHeaders) throws IOException {
        ...
        if (!this.closed) {
            ...
            this.sink.mo12599writeInt(i2 & Integer.MAX_VALUE);
    ...
    public final synchronized void settings(@NotNull Settings settings) throws IOException {
        ...
        if (!this.closed) {
            ...
            while (i < 10) {
                if (settings.isSet(i)) {
                    this.sink.mo12603writeShort(i != 4 ? i != 7 ? i : 4 : 3);
                    this.sink.mo12599writeInt(settings.get(i));
                }
                i++;
            }
            this.sink.flush();
    ...
    public final synchronized void windowUpdate(int i, long j) throws IOException {
        ...
        if (j != 0 && j <= 2147483647L) {
            frameHeader(i, 4, 8, 0);
            this.sink.mo12599writeInt((int) j);
            this.sink.flush();
        } else {
