...
public final class Stats implements Serializable {
    ...
    public double populationVariance() {
        ...
        return this.count == 1 ? FrostVideoEffectController.VIDEO_STRENGTH_CLEAR : DoubleUtils.ensureNonNegative(this.sumOfSquaresOfDeltas) / count();
    }
    ...
    public String toString() {
        if (count() > 0) {
            return MoreObjects.toStringHelper(this).add("count", this.count).add("mean", this.mean).add("populationStandardDeviation", populationStandardDeviation()).add(ReactProperties.GEOFENCE_MINIMUM_RANGE, this.min).add(ReactProperties.GEOFENCE_MAXIMUM_RANGE, this.max).toString();
        }
    ...
    public static double meanOf(Iterator<? extends Number> it2) {
        ...
        while (it2.hasNext()) {
            ...
            j++;
            doubleValue = (!Doubles.isFinite(doubleValue2) || !Doubles.isFinite(doubleValue)) ? StatsAccumulator.calculateNewMeanNonFinite(doubleValue, doubleValue2) : ((doubleValue2 - doubleValue) / j) + doubleValue;
        }
        return doubleValue;
    }
    ...
    public static double meanOf(double... dArr) {
        ...
        for (int i = 1; i < dArr.length; i++) {
            double d2 = dArr[i];
            d = (!Doubles.isFinite(d2) || !Doubles.isFinite(d)) ? StatsAccumulator.calculateNewMeanNonFinite(d, d2) : ((d2 - d) / (i + 1)) + d;
        }
        return d;
    }
