...
public class SmallSortedMap<K extends Comparable<K>, V> extends AbstractMap<K, V> {
    ...
    public class Entry implements Map.Entry<K, V>, Comparable<SmallSortedMap<K, V>.Entry> {
        ...
        public int compareTo(SmallSortedMap<K, V>.Entry entry) {
            return mo9773getKey().compareTo(entry.mo9773getKey());
        }
    ...
    private int binarySearchInArray(K k) {
        ...
        if (size >= 0) {
            int compareTo = k.compareTo(this.entryList.get(size).mo9773getKey());
            if (compareTo > 0) {
                return -(size + 2);
            }
            if (compareTo == 0) {
        ...
        while (i <= size) {
            int i2 = (i + size) / 2;
            int compareTo2 = k.compareTo(this.entryList.get(i2).mo9773getKey());
            if (compareTo2 < 0) {
                size = i2 - 1;
    ...
    public V put(K k, V v) {
        ...
        if (size == i2) {
            ...
            getOverflowEntriesMutable().put((K) remove.mo9773getKey(), remove.getValue());
        }
