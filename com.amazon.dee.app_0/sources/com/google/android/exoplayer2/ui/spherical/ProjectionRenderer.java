...
final class ProjectionRenderer {
    ...
    private static final String[] VERTEX_SHADER_CODE = {"uniform mat4 uMvpMatrix;", "uniform mat3 uTexMatrix;", "attribute vec4 aPosition;", "attribute vec2 aTexCoords;", "varying vec2 vTexCoords;", "void main() {", "  gl_Position = uMvpMatrix * aPosition;", "  vTexCoords = (uTexMatrix * vec3(aTexCoords, 1)).xy;", EntertainmentConstants.TCOMM_PAYLOAD_DESERIALIZED_CLOSE};
    private static final String[] FRAGMENT_SHADER_CODE = {"#extension GL_OES_EGL_image_external : require", "precision mediump float;", "uniform samplerExternalOES uTexture;", "varying vec2 vTexCoords;", "void main() {", "  gl_FragColor = texture2D(uTexture, vTexCoords);", EntertainmentConstants.TCOMM_PAYLOAD_DESERIALIZED_CLOSE};
    ...
    public void draw(int i, float[] fArr, boolean z) {
        ...
        GlUtil.checkGlError();
        GLES20.glVertexAttribPointer(this.positionHandle, 3, (int) FujifilmMakernoteDirectory.TAG_MAX_APERTURE_AT_MIN_FOCAL, false, 12, (Buffer) meshData.vertexBuffer);
        GlUtil.checkGlError();
        GLES20.glVertexAttribPointer(this.texCoordsHandle, 2, (int) FujifilmMakernoteDirectory.TAG_MAX_APERTURE_AT_MIN_FOCAL, false, 8, (Buffer) meshData.textureBuffer);
        GlUtil.checkGlError();
