...
public final class Tx3gDecoder extends SimpleSubtitleDecoder {
    ...
    public Tx3gDecoder(List<byte[]> list) {
        ...
        if (list.size() == 1 && (list.get(0).length == 48 || list.get(0).length == 53)) {
            ...
            if (this.customVerticalPlacement) {
                this.defaultVerticalPlacement = Util.constrainValue(((bArr[11] & 255) | ((bArr[10] & 255) << 8)) / this.calculatedVideoTrackHeight, 0.0f, 0.95f);
                return;
            } else {
    ...
    private void applyStyleRecord(ParsableByteArray parsableByteArray, SpannableStringBuilder spannableStringBuilder) throws SubtitleDecoderException {
        ...
        if (readUnsignedShort2 > spannableStringBuilder.length()) {
            StringBuilder outline109 = GeneratedOutlineSupport1.outline109("Truncating styl end (", readUnsignedShort2, ") to cueText.length() (");
            outline109.append(spannableStringBuilder.length());
            outline109.append(").");
            Log.w(TAG, outline109.toString());
            i = spannableStringBuilder.length();
        } else {
            i = readUnsignedShort2;
        }
        if (readUnsignedShort >= i) {
            Log.w(TAG, GeneratedOutlineSupport1.outline54("Ignoring styl with start (", readUnsignedShort, ") >= end (", i, ")."));
            return;
        }
        int i2 = i;
