...
public class DownsampleUtil {
    ...
    public static int determineSampleSizeJPEG(final EncodedImage encodedImage, final int pixelSize, final int maxBitmapSizeInBytes) {
        ...
        while ((((encodedImage.getHeight() * encodedImage.getWidth()) * pixelSize) / sampleSize) / sampleSize > maxBitmapSizeInBytes) {
            sampleSize *= 2;
        }
        return sampleSize;
    }
    ...
    public static int ratioToSampleSize(final float ratio) {
        ...
        while (true) {
            double d = i;
            if (((1.0d / (Math.pow(d, 2.0d) - d)) * 0.3333333432674408d) + (1.0d / d) <= ratio) {
                return i - 1;
            }
            i++;
        }
    ...
    public static int ratioToSampleSizeJPEG(final float ratio) {
        ...
        while (true) {
            int i2 = i * 2;
            double d = 1.0d / i2;
            if ((0.3333333432674408d * d) + d <= ratio) {
                return i;
            }
            i = i2;
        }
