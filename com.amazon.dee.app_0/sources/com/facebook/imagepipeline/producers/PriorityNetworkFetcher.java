...
public class PriorityNetworkFetcher<FETCH_STATE extends FetchState> implements NetworkFetcher<PriorityFetchState<FETCH_STATE>> {
    ...
    private void moveDelayedRequestsToPriorityQueues() {
        ...
        while (it2.hasNext()) {
            PriorityFetchState<FETCH_STATE> next = it2.next();
    ...
    public void requeue(PriorityFetchState<FETCH_STATE> fetchState) {
        synchronized (this.mLock) {
            ...
            fetchState.delegatedState = this.mDelegate.mo6923createFetchState(fetchState.getConsumer(), fetchState.getContext());
            this.mCurrentlyFetching.remove(fetchState);
            if (!this.mHiPriQueue.remove(fetchState)) {
                this.mLowPriQueue.remove(fetchState);
            }
    ...
    public /* bridge */ /* synthetic */ FetchState mo6923createFetchState(Consumer consumer, ProducerContext producerContext) {
        return mo6923createFetchState((Consumer<EncodedImage>) consumer, producerContext);
    }
    ...
    public PriorityFetchState<FETCH_STATE> mo6923createFetchState(Consumer<EncodedImage> consumer, ProducerContext producerContext) {
        return new PriorityFetchState<>(consumer, producerContext, this.mDelegate.mo6923createFetchState(consumer, producerContext), this.mClock.now(), this.mHiPriQueue.size(), this.mLowPriQueue.size(), this.mCurrentlyFetching.size());
    }
    ...
    public Map<String, String> getExtraMap(PriorityFetchState<FETCH_STATE> fetchState, int byteSize) {
        ...
        outline107.append(fetchState.dequeuedTimestamp - fetchState.enqueuedTimestamp);
        hashMap.put("pri_queue_time", outline107.toString());
