...
public class PartialDiskCacheProducer implements Producer<EncodedImage> {
    ...
    public static class PartialDiskCacheConsumer extends DelegatingConsumer<EncodedImage, EncodedImage> {
        ...
        private void copy(InputStream from, OutputStream to, int length) throws IOException {
            ...
            while (i > 0) {
                try {
                    int read = from.read(mo6905get, 0, Math.min(16384, i));
                    if (read < 0) {
                        break;
                    } else if (read > 0) {
        ...
        private PooledByteBufferOutputStream merge(EncodedImage initialData, EncodedImage remainingData) throws IOException {
            ...
            PooledByteBufferOutputStream mo6912newOutputStream = this.mPooledByteBufferFactory.mo6912newOutputStream(remainingData.getSize() + i);
            copy(initialData.getInputStreamOrThrow(), mo6912newOutputStream, i);
            copy(remainingData.getInputStreamOrThrow(), mo6912newOutputStream, remainingData.getSize());
            return mo6912newOutputStream;
        }
        ...
        public void onNewResultImpl(@Nullable EncodedImage newResult, int status) {
            ...
            } else if (BaseConsumer.statusHasFlag(status, 8) && BaseConsumer.isLast(status) && newResult != null && newResult.getImageFormat() != ImageFormat.UNKNOWN) {
    ...
    private Continuation<EncodedImage, Void> onFinishDiskReads(final Consumer<EncodedImage> consumer, final ProducerContext producerContext, final CacheKey partialImageCacheKey) {
        ...
        return new Continuation<EncodedImage, Void>() { // from class: com.facebook.imagepipeline.producers.PartialDiskCacheProducer.1
            ...
            public Void mo6922then(Task<EncodedImage> task) throws Exception {
