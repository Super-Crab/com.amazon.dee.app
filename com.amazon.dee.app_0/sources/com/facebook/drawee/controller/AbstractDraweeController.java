...
public abstract class AbstractDraweeController<T, INFO> implements DraweeController, DeferredReleaser.Releasable, GestureDetector.ClickListener {
    ...
    protected T mo6874getCachedImage() {
        return null;
    }
    ...
    protected String getImageClass(@Nullable T image) {
        return image != null ? image.getClass().getSimpleName() : DefaultRecordChecker.Regex.EMPTY;
    }
    ...
    protected void submitRequest() {
        ...
        T mo6874getCachedImage = mo6874getCachedImage();
        if (mo6874getCachedImage != null) {
            if (FrescoSystrace.isTracing()) {
            ...
            reportSubmit(this.mDataSource, getImageInfo(mo6874getCachedImage));
            onImageLoadedFromCacheImmediately(this.mId, mo6874getCachedImage);
            onNewResultInternal(this.mId, this.mDataSource, mo6874getCachedImage, 1.0f, true, true, true);
            if (FrescoSystrace.isTracing()) {
                FrescoSystrace.endSection();
            }
            if (!FrescoSystrace.isTracing()) {
        ...
        this.mDataSource.subscribe(new BaseDataSubscriber<T>() { // from class: com.facebook.drawee.controller.AbstractDraweeController.2
            ...
            public void onNewResultImpl(DataSource<T> dataSource) {
                ...
                T mo6898getResult = dataSource.mo6898getResult();
                if (mo6898getResult != null) {
                    AbstractDraweeController.this.onNewResultInternal(str, dataSource, mo6898getResult, progress, isFinished, hasResult, hasMultipleResults);
                } else if (!isFinished) {
                } else {
    ...
    public String toString() {
        return Objects.toStringHelper(this).add("isAttached", this.mIsAttached).add("isRequestSubmitted", this.mIsRequestSubmitted).add("hasFetchFailed", this.mHasFetchFailed).add("fetchedImage", getImageHash(this.mFetchedImage)).add(DefaultDeliveryClient.EVENTS_DIRECTORY, this.mEventTracker.toString()).toString();
    }
