...
import com.amazon.photos.uploader.blockers.PowerSaverBlocker;
import com.amazon.photos.uploader.blockers.QueueBlockerEvaluator;
import kotlin.Metadata;
import kotlin.jvm.internal.Intrinsics;
import org.jetbrains.annotations.NotNull;
import org.jetbrains.annotations.Nullable;
/* compiled from: BatteryStateBlockerEvaluator.kt */
...
public final class BatteryStateBlockerEvaluator implements QueueBlockerEvaluator {
    private final BatteryState batteryState;
    private final SchedulingCallback schedulingCallback;

    public BatteryStateBlockerEvaluator(@NotNull BatteryState batteryState, @NotNull SchedulingCallback schedulingCallback) {
    ...
    public Blocker getBlocker(@NotNull Queue queue) {
        ...
        if (Intrinsics.areEqual(queryBlocker, LowBatteryBlocker.INSTANCE)) {
            this.schedulingCallback.reevaluateBlockers$AndroidPhotosUploader_release(new Constraints.Builder().setRequiresBatteryNotLow(true).build());
    ...
    public Blocker queryBlocker(@NotNull Queue queue) {
        Intrinsics.checkParameterIsNotNull(queue, "queue");
        if (queue.getConstraints().contains(QueueConstraint.BlockInPowerSavingMode.INSTANCE) && this.batteryState.isInPowerSaverMode()) {
            return PowerSaverBlocker.INSTANCE;
        }
        if (queue.getConstraints().contains(QueueConstraint.BlockInLowBattery.INSTANCE) && !this.batteryState.isCharging() && this.batteryState.getBatteryPercent() <= 15) {
            return LowBatteryBlocker.INSTANCE;
        }
        return null;
    }
}
