...
public class ReflectionHelper {
    ...
    public <T> T createInstance(Class<T> cls) {
        try {
            ...
            log.info("createInstance", String.format("IllegalAccessException creating class %s with default constructor.", cls.getName()), new Object[0]);
            return null;
        } catch (InstantiationException unused2) {
            log.info("createInstance", String.format("InstantiationException creating class %s with default constructor.", cls.getName()), new Object[0]);
            return null;
        }
    }
    ...
    public <T> Method getHiddenMethod(Class<T> cls, String str) {
        ...
        if (method == null) {
            log.error("getHiddenMethod", GeneratedOutlineSupport1.outline38(cls, GeneratedOutlineSupport1.outline115("Failed to find method ", str, " in class ")), new Object[0]);
        }
        return method;
    }
    ...
    public Object invokeStatic(String str, String str2, Class<?>[] clsArr, Object... objArr) {
        try {
            ...
            log.error("invokeStatic", GeneratedOutlineSupport1.outline72("Cannot find class ", str2), new Object[0]);
            return null;
        }
    }
    ...
    private Object invoke(String str, Class<?> cls, Object obj, Class<?>[] clsArr, Object... objArr) {
        try {
            ...
            log.error("invoke", "IllegalAccessException calling method", e);
            return null;
        } catch (IllegalArgumentException e2) {
            log.error("invoke", "IllegalArgumentException calling method", e2);
            return null;
        } catch (NoSuchMethodException e3) {
            log.error("invoke", "Method cannot be found. Are you sure it is public?", e3);
            return null;
        } catch (SecurityException e4) {
            log.error("invoke", e4.getMessage(), e4);
            return null;
        } catch (InvocationTargetException e5) {
            log.error("invoke", String.format("Exception thrown while calling method %s", str), e5.getCause());
            log.error("invoke", "Exception calling method", e5);
            return null;
        }
    }
    ...
    public <T> T createInstance(Class<T> cls, List<Class<?>> list, List<?> list2) {
        ...
        if (clsArr.length == array.length) {
            ...
            try {
                ...
                log.error("createInstance", String.format("Could not create class %s because the constructor was not visible.", cls.getName()), e);
                return null;
            } catch (IllegalArgumentException e2) {
                log.error("createInstance", String.format("Could not create class %s because of an illegal argument.", cls.getName()), e2);
                return null;
            } catch (InstantiationException e3) {
                log.error("createInstance", String.format("Could not create class %s because of an instantiation exception.", cls.getName()), e3);
                return null;
            } catch (NoSuchMethodException unused) {
                log.info("createInstance", String.format("Could not create class %s because there was no such constructor.", cls.getName()), new Object[0]);
                return null;
            } catch (SecurityException e4) {
                log.error("createInstance", String.format("Could not create class %s because we did not have permission", cls.getName()), e4);
                return null;
            } catch (InvocationTargetException e5) {
                log.error("createInstance", String.format("Could not create class %s because constructor threw an exception", cls.getName()), e5);
                return null;
            }
        }
