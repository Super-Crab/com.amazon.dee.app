...
public class CryptoModuleDispatcher extends S3CryptoModule<MultipartUploadContext> {
    ...
    public CryptoModuleDispatcher(AWSKMSClient aWSKMSClient, S3Direct s3Direct, AWSCredentialsProvider aWSCredentialsProvider, EncryptionMaterialsProvider encryptionMaterialsProvider, CryptoConfiguration cryptoConfiguration) {
        CryptoConfiguration m6719clone = cryptoConfiguration.m6719clone();
        if (m6719clone.getCryptoMode() == null) {
            m6719clone.setCryptoMode(CryptoMode.EncryptionOnly);
        }
        CryptoConfiguration readOnly = m6719clone.readOnly();
        this.defaultCryptoMode = readOnly.getCryptoMode();
        int ordinal = this.defaultCryptoMode.ordinal();
        if (ordinal == 0) {
            this.eo = new S3CryptoModuleEO(aWSKMSClient, s3Direct, aWSCredentialsProvider, encryptionMaterialsProvider, readOnly);
            CryptoConfiguration m6719clone2 = readOnly.m6719clone();
            try {
                m6719clone2.setCryptoMode(CryptoMode.AuthenticatedEncryption);
            } catch (UnsupportedOperationException unused) {
            }
            this.ae = new S3CryptoModuleAE(aWSKMSClient, s3Direct, aWSCredentialsProvider, encryptionMaterialsProvider, m6719clone2.readOnly());
        } else if (ordinal == 1) {
