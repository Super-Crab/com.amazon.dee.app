...
class NamespaceRemovingInputStream extends SdkFilterInputStream {
    ...
    private int matchXmlNamespaceAttribute(String str) {
        ...
        if (!stringPrefixSlicer.removePrefix(Config.Compare.EQUAL_TO)) {
            return -1;
        }
        stringPrefixSlicer.removeRepeatingPrefix(" ");
        if (!stringPrefixSlicer.removePrefix(EntertainmentConstants.PLAYER_INFO_JSON_ATTRIBUTE_PLAYER_INFO_PAYLOAD_DESERIALIZED) || !stringPrefixSlicer.removePrefixEndingWith(EntertainmentConstants.PLAYER_INFO_JSON_ATTRIBUTE_PLAYER_INFO_PAYLOAD_DESERIALIZED)) {
            return -1;
        }
