...
public class ByteBufferGifDecoder implements ResourceDecoder<ByteBuffer, GifDrawable> {
    ...
    private static int getSampleSize(GifHeader gifHeader, int i, int i2) {
        ...
        if (Log.isLoggable(TAG, 2) && max > 1) {
            StringBuilder outline110 = GeneratedOutlineSupport1.outline110("Downsampling GIF, sampleSize: ", max, ", target dimens: [", i, ReactProperties.HereMapMarker.X);
            outline110.append(i2);
            outline110.append("], actual dimens: [");
            outline110.append(gifHeader.getWidth());
            outline110.append(ReactProperties.HereMapMarker.X);
            outline110.append(gifHeader.getHeight());
            outline110.append("]");
            outline110.toString();
        }
        return max;
        ...
        try {
            ...
            if (parseHeader.getNumFrames() > 0 && parseHeader.getStatus() == 0) {
                ...
                if (Log.isLoggable(TAG, 2)) {
                    StringBuilder outline107 = GeneratedOutlineSupport1.outline107("Decoded GIF from stream in ");
                    outline107.append(LogTime.getElapsedMillis(logTime));
                    outline107.toString();
                }
                return gifDrawableResource;
            }
            if (Log.isLoggable(TAG, 2)) {
                StringBuilder outline1072 = GeneratedOutlineSupport1.outline107("Decoded GIF from stream in ");
                outline1072.append(LogTime.getElapsedMillis(logTime));
                outline1072.toString();
            }
            return null;
        } finally {
            if (Log.isLoggable(TAG, 2)) {
                StringBuilder outline1073 = GeneratedOutlineSupport1.outline107("Decoded GIF from stream in ");
                outline1073.append(LogTime.getElapsedMillis(logTime));
