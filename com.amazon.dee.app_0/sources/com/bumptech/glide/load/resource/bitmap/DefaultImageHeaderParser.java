...
public final class DefaultImageHeaderParser implements ImageHeaderParser {
    ...
    private int moveToExifSegmentAndGetLength(Reader reader) throws IOException {
        ...
        do {
            ...
            if (uInt82 != 255) {
                if (Log.isLoggable(TAG, 3)) {
                    GeneratedOutlineSupport1.outline149("Unknown segmentId=", uInt82);
                }
                return -1;
            }
        ...
        if (Log.isLoggable(TAG, 3)) {
            StringBuilder outline110 = GeneratedOutlineSupport1.outline110("Unable to skip enough data, type: ", uInt8, ", wanted to skip: ", uInt16, ", but actually skipped: ");
            outline110.append(skip);
            outline110.toString();
        }
        return -1;
    }
    ...
    private int getOrientation(Reader reader, ArrayPool arrayPool) throws IOException {
        ...
        if (!handles(uInt16)) {
            if (Log.isLoggable(TAG, 3)) {
                GeneratedOutlineSupport1.outline149("Parser doesn't handle magic number: ", uInt16);
            }
            return -1;
        }
    ...
    private static int parseExifSegment(RandomAccessReader randomAccessReader) {
        ...
        if (int16 == INTEL_TIFF_MAGIC_NUMBER) {
            ...
            if (Log.isLoggable(TAG, 3)) {
                GeneratedOutlineSupport1.outline149("Unknown endianness = ", int16);
            }
            ...
            if (int163 == 274) {
                ...
                if (int164 >= 1 && int164 <= 12) {
                    ...
                    if (int322 < 0) {
                        ...
                        if (Log.isLoggable(TAG, 3)) {
                            StringBuilder outline110 = GeneratedOutlineSupport1.outline110("Got tagIndex=", i, " tagType=", int163, " formatCode=");
                            outline110.append((int) int164);
                            outline110.append(" componentCount=");
                            outline110.append(int322);
                            outline110.toString();
                        }
                        int i2 = int322 + BYTES_PER_FORMAT[int164];
                        if (i2 > 4) {
                            if (Log.isLoggable(TAG, 3)) {
                                GeneratedOutlineSupport1.outline149("Got byte count > 4, not orientation, continuing, formatCode=", int164);
                            }
                        } else {
                            ...
                            if (i3 >= 0 && i3 <= randomAccessReader.length()) {
                                ...
                                if (Log.isLoggable(TAG, 3)) {
                                    GeneratedOutlineSupport1.outline149("Illegal number of bytes for TI tag data tagType=", int163);
                                }
                            } else if (Log.isLoggable(TAG, 3)) {
                                String str = "Illegal tagValueOffset=" + i3 + " tagType=" + ((int) int163);
                            }
                        }
                    }
                } else if (Log.isLoggable(TAG, 3)) {
                    GeneratedOutlineSupport1.outline149("Got invalid format code = ", int164);
                }
            }
        }
        return -1;
