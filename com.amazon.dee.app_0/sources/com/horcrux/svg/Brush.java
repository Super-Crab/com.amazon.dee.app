...
public class Brush {
    ...
    private double getVal(SVGLength sVGLength, double d, float f, float f2) {
        ...
        if (this.mUseObjectBoundingBox && sVGLength.unit == SVGLength.UnitType.NUMBER) {
            d2 = d;
            return PropHelper.fromRelative(sVGLength, d, FrostVideoEffectController.VIDEO_STRENGTH_CLEAR, d2, f2);
        }
        d2 = f;
        return PropHelper.fromRelative(sVGLength, d, FrostVideoEffectController.VIDEO_STRENGTH_CLEAR, d2, f2);
    }
    ...
    public void setupPaint(Paint paint, RectF rectF, float f, float f2) {
        ...
        if (brushType == BrushType.LINEAR_GRADIENT) {
            ...
            Shader radialGradient = new RadialGradient((float) (getVal(this.mPoints[4], d7, f, textSize) + f3), (float) ((f4 / val7) + getVal(this.mPoints[5], d8 / val7, f, textSize)), (float) val6, iArr, fArr, Shader.TileMode.CLAMP);
