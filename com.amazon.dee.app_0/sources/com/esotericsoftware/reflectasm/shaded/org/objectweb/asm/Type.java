...
public class Type {
                ...
                if (c != '[') {
                    switch (c) {
                        ...
                        case 'D':
                            return DOUBLE_TYPE;
                        default:
                            return new Type(11, cArr, 0, cArr.length);
                    }
                }
                int i4 = 1;
                while (true) {
    ...
    private static void a(StringBuffer stringBuffer, Class cls) {
        ...
        stringBuffer.append(cls == Integer.TYPE ? 'I' : cls == Void.TYPE ? 'V' : cls == Boolean.TYPE ? Matrix.MATRIX_TYPE_ZERO : cls == Byte.TYPE ? 'B' : cls == Character.TYPE ? 'C' : cls == Short.TYPE ? 'S' : cls == Double.TYPE ? 'D' : cls == Float.TYPE ? 'F' : 'J');
    }
