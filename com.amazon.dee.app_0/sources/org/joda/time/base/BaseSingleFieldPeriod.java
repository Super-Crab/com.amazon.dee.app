...
public abstract class BaseSingleFieldPeriod implements ReadablePeriod, Comparable<BaseSingleFieldPeriod>, Serializable {
    ...
    public static int standardPeriodIn(ReadablePeriod readablePeriod, long j) {
        ...
        for (int i = 0; i < readablePeriod.size(); i++) {
            ...
            if (value != 0) {
                ...
                if (!field.isPrecise()) {
                    StringBuilder outline107 = GeneratedOutlineSupport1.outline107("Cannot convert period to duration as ");
                    outline107.append(field.getName());
                    outline107.append(" is not precise in the period ");
                    outline107.append(readablePeriod);
    ...
    @Override // org.joda.time.ReadablePeriod
    public int hashCode() {
        return getFieldType().hashCode() + ((459 + getValue()) * 27);
    }
