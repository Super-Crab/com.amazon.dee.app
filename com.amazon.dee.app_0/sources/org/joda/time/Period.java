...
public final class Period extends BasePeriod implements ReadablePeriod, Serializable {
    ...
    private void checkYearsAndMonths(String str) {
        if (getMonths() == 0) {
            if (getYears() != 0) {
                throw new UnsupportedOperationException(GeneratedOutlineSupport1.outline75("Cannot convert to ", str, " as this period contains years and years vary in length"));
            }
            return;
        }
        throw new UnsupportedOperationException(GeneratedOutlineSupport1.outline75("Cannot convert to ", str, " as this period contains months and months vary in length"));
    }
    ...
    public Period normalizedStandard(PeriodType periodType) {
        ...
        Period period = new Period((getWeeks() * 604800000) + (getDays() * 86400000) + (getHours() * 3600000) + (getMinutes() * 60000) + (getSeconds() * 1000) + getMillis(), periodType2, ISOChronology.getInstanceUTC());
        int years = getYears();
        ...
        if (years != 0 || months != 0) {
            ...
            if (j != 0) {
                StringBuilder outline107 = GeneratedOutlineSupport1.outline107("Unable to normalize as PeriodType is missing either years or months but period has a month/year amount: ");
    ...
    public Days toStandardDays() {
        ...
        return Days.days(FieldUtils.safeToInt(FieldUtils.safeAdd(FieldUtils.safeAdd(((getHours() * 3600000) + ((getMinutes() * 60000) + ((getSeconds() * 1000) + getMillis()))) / 86400000, getDays()), getWeeks() * 7)));
    }

    public Duration toStandardDuration() {
        checkYearsAndMonths(Constants.CALL_DURATION_KEY);
    ...
    public Hours toStandardHours() {
        ...
        return Hours.hours(FieldUtils.safeToInt(FieldUtils.safeAdd(FieldUtils.safeAdd(FieldUtils.safeAdd(((getMinutes() * 60000) + ((getSeconds() * 1000) + getMillis())) / 3600000, getHours()), getDays() * 24), getWeeks() * 168)));
    }

    public Minutes toStandardMinutes() {
        checkYearsAndMonths("Minutes");
        return Minutes.minutes(FieldUtils.safeToInt(FieldUtils.safeAdd(FieldUtils.safeAdd(FieldUtils.safeAdd(FieldUtils.safeAdd(((getSeconds() * 1000) + getMillis()) / 60000, getMinutes()), getHours() * 60), getDays() * 1440), getWeeks() * 10080)));
    }
    ...
    public Weeks toStandardWeeks() {
        ...
        return Weeks.weeks(FieldUtils.safeToInt((((getDays() * 86400000) + ((getHours() * 3600000) + ((getMinutes() * 60000) + ((getSeconds() * 1000) + getMillis())))) / 604800000) + getWeeks()));
    }
