    ...
    public byte[] generateSignature(byte[] bArr) {
        ...
        if (bArr != null) {
            ...
            synchronized (xMSSMTPrivateKeyParameters) {
                ...
                build.getReducedSignatures().add(new XMSSReducedSignature.Builder(this.xmssParams).withWOTSPlusSignature(wotsSign(HMsg, oTSHashAddress)).withAuthPath(bDSState.get(0).getAuthenticationPath()).mo12865build());
                for (int i = 1; i < this.params.getLayers(); i++) {
                    ...
                    build.getReducedSignatures().add(new XMSSReducedSignature.Builder(this.xmssParams).withWOTSPlusSignature(wotsSign).withAuthPath(bDSState.get(i).getAuthenticationPath()).mo12865build());
                }
