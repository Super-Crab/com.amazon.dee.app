    ...
    public PGPSignature generate() throws PGPException {
        ...
        try {
            ...
            if (this.contentSigner.getKeyAlgorithm() == 3 || this.contentSigner.getKeyAlgorithm() == 1) {
                ...
                mPIntegerArr2 = new MPInteger[]{new MPInteger(new BigInteger(1, Arrays.copyOfRange(signature, 0, signature.length / 2))), new MPInteger(new BigInteger(1, Arrays.copyOfRange(signature, signature.length / 2, signature.length)))};
                byte[] digest = this.contentSigner.getDigest();
                return new PGPSignature(new SignaturePacket(this.sigType, this.contentSigner.getKeyID(), this.contentSigner.getKeyAlgorithm(), this.contentSigner.getHashAlgorithm(), insertSubpacket, insertSubpacket2, new byte[]{digest[0], digest[1]}, mPIntegerArr2));
            } else {
                mPIntegerArr = PGPUtil.dsaSigToMpi(this.contentSigner.getSignature());
            }
            mPIntegerArr2 = mPIntegerArr;
            byte[] digest2 = this.contentSigner.getDigest();
            return new PGPSignature(new SignaturePacket(this.sigType, this.contentSigner.getKeyID(), this.contentSigner.getKeyAlgorithm(), this.contentSigner.getHashAlgorithm(), insertSubpacket, insertSubpacket2, new byte[]{digest2[0], digest2[1]}, mPIntegerArr2));
        } catch (IOException e) {
