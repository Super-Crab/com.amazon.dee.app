...
public class JcaPGPKeyConverter {
    ...
    private BCPGKey getPublicBCPGKey(int i, PGPAlgorithmParameters pGPAlgorithmParameters, PublicKey publicKey, Date date) throws PGPException {
        if (publicKey instanceof RSAPublicKey) {
            ...
            if (i == 18) {
                PGPKdfParameters implGetKdfParameters = implGetKdfParameters(pGPAlgorithmParameters);
                return new ECDHPublicBCPGKey(aSN1ObjectIdentifier, x9ECPoint.getPoint(), implGetKdfParameters.getHashAlgorithm(), implGetKdfParameters.getSymmetricWrapAlgorithm());
            } else if (i != 19) {
            ...
            bArr[0] = 64;
            System.arraycopy(subjectPublicKeyInfo2.getPublicKeyData().getBytes(), 0, bArr, 1, bArr.length - 1);
            return new EdDSAPublicBCPGKey(GNUObjectIdentifiers.Ed25519, new BigInteger(1, bArr));
        } else if (!publicKey.getAlgorithm().regionMatches(true, 0, "X2", 0, 2)) {
            throw new PGPException("unknown key class");
        } else {
            SubjectPublicKeyInfo subjectPublicKeyInfo3 = SubjectPublicKeyInfo.getInstance(publicKey.getEncoded());
            byte[] bArr2 = new byte[33];
            bArr2[0] = 64;
            System.arraycopy(subjectPublicKeyInfo3.getPublicKeyData().getBytes(), 0, bArr2, 1, bArr2.length - 1);
            PGPKdfParameters implGetKdfParameters2 = implGetKdfParameters(pGPAlgorithmParameters);
            return new ECDHPublicBCPGKey(CryptlibObjectIdentifiers.curvey25519, new BigInteger(1, bArr2), implGetKdfParameters2.getHashAlgorithm(), implGetKdfParameters2.getSymmetricWrapAlgorithm());
        }
    }
    ...
    private PublicKey implGetPublicKeyEC(String str, ECPublicBCPGKey eCPublicBCPGKey) throws GeneralSecurityException, IOException, PGPException {
        ...
        ECPoint decodePoint = JcaJcePGPUtil.decodePoint(eCPublicBCPGKey.getEncodedPoint(), x9Parameters.getCurve());
        return implGeneratePublic(str, new ECPublicKeySpec(new java.security.spec.ECPoint(decodePoint.getAffineXCoord().toBigInteger(), decodePoint.getAffineYCoord().toBigInteger()), getECParameterSpec(curveOID, x9Parameters)));
    }
    ...
    public PublicKey getPublicKey(PGPPublicKey pGPPublicKey) throws PGPException {
        ...
        try {
            ...
            if (algorithm == 1 || algorithm == 2 || algorithm == 3) {
                ...
                return implGeneratePublic(KeyUtils.ALGORITHM_RSA, new RSAPublicKeySpec(rSAPublicBCPGKey.getModulus(), rSAPublicBCPGKey.getPublicExponent()));
            } else if (algorithm == 22) {
                ...
                if (asUnsignedByteArray.length >= 1 && 64 == asUnsignedByteArray[0]) {
                    return implGetPublicKeyX509("EdDSA", new SubjectPublicKeyInfo(new AlgorithmIdentifier(EdECObjectIdentifiers.id_Ed25519), Arrays.copyOfRange(asUnsignedByteArray, 1, asUnsignedByteArray.length)));
                }
                ...
                switch (algorithm) {
                    ...
                    case 18:
                        ...
                        if (asUnsignedByteArray2.length >= 1 && 64 == asUnsignedByteArray2[0]) {
                            return implGetPublicKeyX509("XDH", new SubjectPublicKeyInfo(new AlgorithmIdentifier(EdECObjectIdentifiers.id_X25519), Arrays.copyOfRange(asUnsignedByteArray2, 1, asUnsignedByteArray2.length)));
                        }
