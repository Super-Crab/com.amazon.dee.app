...
public class SecT193R2Point extends ECPoint.AbstractF2m {
    ...
    public ECPoint add(ECPoint eCPoint) {
        ...
        if (rawXCoord.isZero()) {
            ...
            ECFieldElement add3 = GeneratedOutlineSupport1.outline139(divide, divide, xCoord).add(curve.getA());
            if (add3.isZero()) {
    ...
    public ECPoint twice() {
        ...
        ECFieldElement outline139 = GeneratedOutlineSupport1.outline139(eCFieldElement2, multiply, a);
        if (outline139.isZero()) {
            return new SecT193R2Point(curve, outline139, curve.getB().sqrt());
        }
        ECFieldElement square2 = outline139.square();
        ECFieldElement multiply2 = isOne ? outline139 : outline139.multiply(square);
        if (!isOne) {
            eCFieldElement = eCFieldElement.multiply(eCFieldElement3);
        }
        return new SecT193R2Point(curve, square2, eCFieldElement.squarePlusProduct(outline139, multiply).add(square2).add(multiply2), new ECFieldElement[]{multiply2});
    }
