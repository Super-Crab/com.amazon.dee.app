...
public class SendSignedAndEncryptedMail {
    ...
    /* JADX WARN: Type inference failed for: r11v3, types: [org.bouncycastle.mail.smime.SMIMEException] */
    public static void main(String[] strArr) {
        ...
        try {
            ...
            mimeMessage.setContent("example encrypted message", Constants.TEXT_PLAIN_MEDIA_TYPE);
            ...
            sMIMESignedGenerator.addSignerInfoGenerator(new JcaSimpleSignerInfoGeneratorBuilder().setProvider(BouncyCastleProvider.PROVIDER_NAME).setSignedAttributeGenerator(new AttributeTable(aSN1EncodableVector)).build("DSA".equals(privateKey.getAlgorithm()) ? "SHA1withDSA" : "MD5withRSA", privateKey, (X509Certificate) certificateChain[0]));
            ...
            MimeBodyPart generate2 = sMIMEEnvelopedGenerator.generate(mimeMessage2, new JceCMSContentEncryptorBuilder(CMSAlgorithm.RC2_CBC).setProvider(BouncyCastleProvider.PROVIDER_NAME).build());
