...
public class PKIXNameConstraintValidator implements NameConstraintValidator {
    ...
    */
    ...
    private Set intersectOtherName(Set set, Set set2) {
        ...
        Iterator it2 = set2.iterator();
        while (it2.hasNext()) {
            OtherName otherName = OtherName.getInstance(((GeneralSubtree) it2.next()).getBase().getName());
            if (set != null) {
                for (Object obj : set) {
    ...
    private String stringifyIPCollection(Set set) {
        ...
        while (it2.hasNext()) {
            ...
            outline107.append(stringifyIP((byte[]) it2.next()));
        }
        outline107.append("]");
    ...
    private String stringifyOtherNameCollection(Set set) {
        ...
        for (Object obj : set) {
            ...
            outline107.append(otherName.getTypeID().getId());
            outline107.append(":");
            try {
                outline107.append(Hex.toHexString(otherName.getValue().toASN1Primitive().getEncoded()));
            } catch (IOException e) {
    ...
    */
        ...
        if (aSN1Sequence2.size() >= 1 && aSN1Sequence2.size() <= aSN1Sequence.size()) {
            ...
            for (int i3 = 0; i3 < aSN1Sequence2.size(); i3++) {
                ...
                if (rdn2.size() != 1 || !rdn2.getFirst().getType().equals((ASN1Primitive) RFC4519Style.serialNumber)) {
                    if (!IETFUtils.rDNAreEqual(rdn2, rdn3)) {
                        return false;
                    }
                } else if (!rdn3.getFirst().getValue().toString().startsWith(rdn2.getFirst().getValue().toString())) {
                    return false;
                }
            }
            return true;
    ...
    public void addExcludedSubtree(GeneralSubtree generalSubtree) {
        ...
        if (tagNo == 0) {
            ...
            StringBuilder outline107 = GeneratedOutlineSupport1.outline107("Unknown tag encountered: ");
            outline107.append(base.getTagNo());
    ...
    public void intersectEmptyPermittedSubtree(int i) {
        if (i == 0) {
            ...
            throw new IllegalStateException(GeneratedOutlineSupport1.outline49("Unknown tag encountered: ", i));
        } else {
    ...
    public void intersectPermittedSubtree(GeneralSubtree[] generalSubtreeArr) {
        ...
        for (Map.Entry entry : hashMap.entrySet()) {
            ...
            if (intValue == 0) {
                ...
                throw new IllegalStateException(GeneratedOutlineSupport1.outline49("Unknown tag encountered: ", intValue));
            } else {
