...
public class CertificateStatus {
    ...
    public static CertificateStatus parse(TlsContext tlsContext, InputStream inputStream) throws IOException {
        ...
        if (readUint8 == 1) {
            ...
            oCSPResponse = OCSPResponse.getInstance(TlsUtils.readDERObject(TlsUtils.readOpaque24(inputStream, 1)));
        } else if (readUint8 != 2) {
            ...
            while (byteArrayInputStream.available() > 0) {
                ...
                vector.addElement(readUint24 < 1 ? null : OCSPResponse.getInstance(TlsUtils.readDERObject(TlsUtils.readFully(readUint24, byteArrayInputStream))));
            }
            vector.trimToSize();
            oCSPResponse = vector;
        }
        ...
        if (s == 1) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                if (oCSPResponse == null) {
                    ...
                    vector2.addElement(oCSPResponse.getEncoded("DER"));
                    j += encoded.length;
                }
                j += 3;
            }
